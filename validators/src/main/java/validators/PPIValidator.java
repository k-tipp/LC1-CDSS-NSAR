					...
					for(ICdss4NsarDrug drug : cdssRequest.getDrugs()) {
						if(drug.isNsar()) {
							warnings.add(Cdss4NsarWarning.create().setName("Warnung PPI")
									.setDescription("Dem Patienten wurden Steroide und NSAR verschrieben, es muss zusätzlich ein PPI verschrieben werden.")
									.setMeasurementType("")
									.setMeasurementUnit("")
									.setMeasurementValue("")
									.setFailedTest("NSAR + Steroid")
						 ...
						 if (cdssRequest.getAge() > 65){
							warnings.add(Cdss4NsarWarning.create().setName("Warnung PPI")
									.setDescription("Der Patient ist älter als 65, es muss zusätzlich ein PPI verschrieben werden.")
									.setMeasurementType("")
									.setMeasurementUnit("")
									.setMeasurementValue("")
									.setFailedTest("NSAR + Alter")
									.setConflictObjOne("Patient")
									.setConflictObjTwo("")
									.setAlertLevel("warning"));
						}
